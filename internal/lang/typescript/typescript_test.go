...
func TestDeclarationsFixture(t *testing.T) {
	src := parse(t, "declarations.ts")
	defer src.Close()

	sigs, err := TypeScript{}.ListSignatures(src)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, s := range sigs {
		text, err := TypeScript{}.ReadDeclaration(src, core.SymbolID{Kind: s.Kind, Name: s.Name, Container: s.Container})
		if err != nil {
