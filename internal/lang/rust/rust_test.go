...
func TestDeclarationsFixture(t *testing.T) {
	src := parse(t, "declarations.rs")

	sigs, err := rust.Rust{}.ListSignatures(src)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, s := range sigs {
		text, err := rust.Rust{}.ReadDeclaration(src, core.SymbolID{Kind: s.Kind, Name: s.Name, Container: s.Container})
		if err != nil {
