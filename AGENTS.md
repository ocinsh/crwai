...
Release tags, package names, and `internal/core/version.go` use `vX.X.X`. The tag workflow in `.github/workflows/release.yml` validates the source version, runs checks, builds four native CGO packages, and publishes checksums. The CLI `install`, `check-update`, and `update` commands live in `cmd/crwai/distribution.go`; their download and verification logic belongs in `internal/release/`. The install wizard prompts for each detected client and registers the resolved path of its current executable without copying it. Update replaces that same executable. Keep `docs/releases.md` aligned with these contracts.
